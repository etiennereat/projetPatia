...
for benchmark in pddl/*; do
    ...
    for problem in $benchmark/p*.pddl; do
        filename=`basename $problem`
        echo "Test for $filename :"
        java -cp classes:lib/pddl4j-3.8.3.jar:lib/sat4j-sat.jar fr.uga.pddl4j.tutorial.asp.ASP -o $benchmark/domain.pddl -f $problem -t 300 -s stat.csv > /dev/null
        echo "done"
    done
done
